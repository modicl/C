...
bool check(char* word)
{
    ...
    for (int i = 0 , n = strlen(word); i < n ; i++ )
    {
        ...
        // We check if the children is NULL
        if (cursor -> children[index] == NULL)
        {
            return false;
        }
        // Cursor is now the children if is not NULL
        else
        {
            cursor = cursor -> children[index];
        }
        // We check if is_word is true AND if we are in the last letter of the word
        if (cursor -> is_word == true && i == n-1)
        {
            return true;
        }
